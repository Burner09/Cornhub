...
export default function ProductCard({ item, onDelete }) {
  ...
  return (
    <div className='w-[420px] p-4 border rounded-lg bg-white'>
      ...
      {showModal && (
        <ConfirmDeleteModal name={item.name} link={`http://localhost:3002/items/${item._id}`} onClose={() => setShowModal(false)} onDelete={handleDelete} />
      )}
    </div>
  );
}